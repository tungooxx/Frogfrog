...
bool Log::check_frog(const Frog& i_frog) const
{
	if (3 > type)
	{
		return i_frog.get_rect().intersects(sf::FloatRect(static_cast<short>(x + floor(0.25f * CELL_SIZE)), y, static_cast<short>(size - floor(0.5f * CELL_SIZE)), CELL_SIZE));
	}
	else
	{
		return i_frog.get_rect().intersects(sf::FloatRect(static_cast<short>(x + floor(0.25f * CELL_SIZE)), y, static_cast<short>(size - floor(2.5f * CELL_SIZE)), CELL_SIZE));
	}
}

void Log::draw(sf::RenderWindow& i_window)
{
	texture.loadFromFile("D:/test2/img/Logs.png");
