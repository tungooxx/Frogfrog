...
bool Turtle::check_frog(const Frog& i_frog) const
{
	...
	{
		return i_frog.get_rect().intersects(sf::FloatRect(static_cast<short>(x + floor(0.25f * CELL_SIZE)), y, static_cast<short>(CELL_SIZE * size - floor(0.5f * CELL_SIZE)), CELL_SIZE));
	}
}

void Turtle::draw(sf::RenderWindow& i_window)
{
	texture.loadFromFile("D:/test2/img/Turtle-1.png");
